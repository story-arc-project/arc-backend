...
@internal_router.post("/comprehensive/failed")
async def failed_comprehensive(body: Annotated[dict[str, Any], Depends(check_internal)], session: SessionDep, response: Response):
    analysis_id: str | None = body.get("analysis_id")
    result: str | None = body.get("result")
    if analysis_id is None or result is None:
        raise AppException(
...
@internal_router.post("/keyword/failed")
async def failed_keyword(body: Annotated[dict[str, Any], Depends(check_internal)], session: SessionDep, response: Response):
    analysis_id: str | None = body.get("analysis_id")
    result: str | None = body.get("result")
    if analysis_id is None or result is None:
        raise AppException(
